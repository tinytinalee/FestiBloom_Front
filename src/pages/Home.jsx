  ...
  return (
    ...
    >
      <div className="home-poster">
        {imageURL ? (
          <img src={imageURL} alt={festival.festivalName} />
        ) : (
...
function Home() {
  ...
  const loadData = () => {
    if (localStorage.getItem("loginState") == "business") {
      axios
        .get(
          // `http://localhost:8080/festival/${localStorage.getItem("loginId")}`
          "http://localhost:8080/festival"
        )
        .then((res) => {
  ...
  const logout = () => {
    ...
    console.log(localStorage.getItem("loginState") == null);
    navigate("/");
  };
    ...
    <div className="home">
      <div className="auth-buttons">
        {localStorage.getItem("loginState") == "notLoggedIn" ? (
          ...
          <>
            <span>{localStorage.getItem("loginName")}님</span>
      ...
      {localStorage.getItem("loginState") == "business" ? (
        <button className="add-festival" onClick={addFestival}>
          페스티벌 등록하기
        </button>
      ) : (
